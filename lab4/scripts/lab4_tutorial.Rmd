...
## Intro
This week we will go over one sample t-tests using two different methods. Using a hypothetical dataset and research hypothesis, we will walk through how to conduct t-tests by hand and then by using the `t.test()` function.
...
# One-sample t-test using the `t.test()` function

##
We can also use the `t.test()` function to avoid doing computations by hand. This is a built-in function we can use to conduct a t-test.  
...
- alternative: this argument can take one of three forms depending on the direction of your alternative hypothesis:  
    - If your alternative hypothesis is non-directional you'll use `two.sided`  
    - If your alternative hypothesis implies a certain direction you'll use either `greater` or `lesser` depending on the direction  
- mu: the population value in your null hypothesis that you are testing against  
...
##
The output for `t.test()` provides us with all the numbers we previously computed by hand.

- t-statistic is reported as 1.9651  
- df is reported as 87  
- p-value is reported as 0.02629  

Just like before, we use these values and our set alpha-value to reject the null hypothesis.

## Cohen's d

You can use the values generated from `t.test()` to compute Cohen's d as an effect size estimate. Remember Cohen's d is calculated as:
$$\frac{(\bar{X} - \mu)}{\hat{\sigma}}$$

We'll do this by hand, first by creating objects with the necessary assigned values like before.
...
deviation of 12.42. A one-sample t-test was conducted to test whether or not the average
ADD score in this sample was significantly higher than 50, the mean ADD score for a
normative population. Based on the results of a one-sample t-test, the sample mean was
found to be significantly higher than the expected score from a typical population, t(87)
=1.965, p = 0.0263, Cohen’s d = 0.21.

# Two-tailed t-test using `t.test()`
...
When we run our analysis, we'll specify the corresponding argument for `alternative`. That is, `alternative = "two.sided"`.
...
Notice our p-value is now 0.05259--double our previous p-value.

Because it is not less than 0.05, we do not reject the null. The summary we write will look different as a result:

The average ADD score for 88 children in this particular sample was 52.60 with a standard
deviation of 12.42. A one-sample t-test was conducted to test whether or not the average
ADD score in this sample was different than 50, the mean ADD score for a normative population. Based on the results of a one-sample t-test, there was not a significant difference, t(87) =1.965, p = 0.053, Cohen’s d = 0.21.
...
The `t.test()` function reports a 95% confidence interval in its calculation. To illustrate this, we will walk through how to calculate a 95% confidence interval by hand.
...
Our resulting 95% confidence interval is identical to the reported values using `t.test()`.